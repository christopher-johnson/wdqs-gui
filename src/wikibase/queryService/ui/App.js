 ...
 */
function App( $element, Mod, Editor, SparqlApi, QuerySamples, Tooltip, QueryExampleDialog, RdfNamespaces ) {
...
App.prototype._getDTOptions = function() {
	...
	options = {
		...
		"aaSorting": [],
		"dom": '<"container"rt<"bottom navbar navbar-default navbar-fixed-bottom"flp>><"clear">'
	};
	return options;
};
...
App.prototype._drawDataTable = function () {
	...
	if (config.data) {
		...
		var pointer = $('#data-table_wrapper');
		pointer.find('.navbar').wrapInner('<ul class="nav navbar-nav">');
		pointer.find('.dataTables_filter').wrap('<li class="navbar-left"></li>');
		pointer.find('.dataTables_paginate').wrap('<li></li>');
		pointer.find('.dataTables_length').wrap('<li class="pull-right"></li>');
	}
};
