module.exports = WikibaseRDFTooltip;
 ...
 * @constructor
 */
function WikibaseRDFTooltip($, editor, mod ) {
	this.editor = editor;
	this.CodeMirror = mod;
	this._registerHandler();
}

WikibaseRDFTooltip.prototype.editor = null;
WikibaseRDFTooltip.prototype.tooltipTimeoutHandler = null;
...
WikibaseRDFTooltip.prototype._registerHandler = function () {
	this.CodeMirror.on( this.editor.getWrapperElement(), 'mouseover', $.proxy( this._triggerTooltip, this ) );
};

WikibaseRDFTooltip.prototype._triggerTooltip = function ( e ) {
...
WikibaseRDFTooltip.prototype._createTooltip = function ( e ) {
	var posX = e.clientX,
...
WikibaseRDFTooltip.prototype._removeToolTip = function () {
	$( '.wikibaseRDFtoolTip' ).remove();
};

WikibaseRDFTooltip.prototype._showToolTip = function ( text, pos ) {
	if ( !text || !pos ) {
		return;
	}
	$( '<div/>' )
...
WikibaseRDFTooltip.prototype._extractPrefixes = function ( text ) {
	var prefixes = {},
		lines = text.split( '\n' ),
		matches;
...
WikibaseRDFTooltip.prototype._searchEntities = function ( term, type ) {
	var entityList = [],
		deferred = $.Deferred();
